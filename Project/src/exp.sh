...
# th mem2_pe.lua -filename all -hops 1 -adjacent 1 -nepochs 1 -pe 1
# 2 hop TE PE adjacent joint (2 times for robustness)
# (estimated time 2 * 100 * 55 s = 3 hours)
for i in `seq 1 2`;
do
    th mem2_pe.lua -filename all -hops 2 -adjacent 1 -nepochs 1 -pe 1 -extension $i
done
# 3 hop TE PE adjacent joint (2 times for robustness)
# (estimated time 100 * 20 s = 35min)
for i in `seq 1 2`;
do
    th mem2_pe.lua -filename all -hops 3 -adjacent 1 -nepochs 1 -pe 1 -extension $i
done

# ######## Models with TE/PE/RNNlike/joint
# (estimated time 100 * 20 s = 35min)
th mem2_pe.lua -filename all -hops 3 -adjacent 0 -nepochs 1 -pe 1
